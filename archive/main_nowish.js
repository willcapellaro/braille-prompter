...
function restart() {
    ...
    timer = setInterval(function() {
        if (stopFlag === false && index < (array.length)) {
            document.getElementById("alphabet-image").src = "imgs/letter-" + array[index] + ".svg";
            globalIndex = index;
            index++;
        } else {
            clearInterval(timer);
...
shortcut.add("Up", 
    function() { 
    document.getElementById("speed-slider").value = document.getElementById("speed-slider").value - 100;
    }, 
    { 'type':'keydown', 'propagate':true, 'target':document} 
    ); 
